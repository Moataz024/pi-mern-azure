...
const AffichageCrud = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('https://link-up-tournament.azurewebsites.net/Payment');
                if (!response.ok) {
    ...
    const handleEditPaymentStatus = async (paymentId, currentStatus) => {
        try {
            ...
            const response = await fetch(`https://link-up-tournament.azurewebsites.net/Payment/${paymentId}`, {
                method: 'PUT',
                headers: {
