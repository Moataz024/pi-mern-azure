...
const AddTeamsWidget = ({ tournamentId }) => {
  ...
  useEffect(() => {
    const fetchTournamentDetails = async () => {
        try {
            const response = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/getbyid/${tournamentIdString}`);
            if (!response.ok) {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        const response = await axios.get('https://link-up-tournament.azurewebsites.net/Team/getall');
  ...
  const [matchestable, setMatches] = useState([]);
    ...
    /*useEffect(() => {
        const fetchMatches = async () => {
            try {
                const response = await fetch('https://link-up-tournament.azurewebsites.net/Tournament/fixtures/65eb22e0767105013a8eaa41');
                if (!response.ok) {
    ...
    const number = selectedTeamIds.length;
      ...
      try {
        ...
        const addTeamsResponse = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/addteams/${tournament._id}`, {
          method: 'PUT',
          headers: {
        ...
        if (tournament.TournamentType === 'League') {
          try {
            const [generateScheduleResponse, standingsResponse] = await Promise.all([
              fetch(`https://link-up-tournament.azurewebsites.net/Tournament/generateRoundRobinSchedule/${tournament._id}`),
              fetch(`https://link-up-tournament.azurewebsites.net/Standings/CreateStandings/${tournament._id}`)
            ]);
          ...
          const generateScheduleResponse = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/KnockoutTournamentBuild/${tournament._id}`);
          if (!generateScheduleResponse.ok) {
        ...
        const generateScheduleResponse = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/ChampionshipGroupsAndMatches/${tournament._id}`);
        if (!generateScheduleResponse.ok) {
