...
try{
        ...
        const StaduimsAndRefs = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/getRefereesAndStadiumsForTournament/${selectedTournamentId}`);
        ...
        try {
            ...
            if (stadiums && stadiums.length > 0) {
                const stadiumPromises = stadiums.map(async (stadiumId) => {
                    const response = await fetch(`https://link-up-tournament.azurewebsites.net/Stadium/getbyid/${stadiumId}`);
                    if (!response.ok) {
            ...
            if (referees && referees.length > 0) {
                const refereePromises = referees.map(async (refereeId) => {
                    const response = await fetch(`https://link-up-tournament.azurewebsites.net/User/getbyid/${refereeId}`);
                    if (!response.ok) {
...
const handleStartDateChange = (e) => {
  ...
  setStartDate({
    ...
    useEffect(() => {
        ...
        try {
          ...
          const team1Response = await fetch(`https://link-up-tournament.azurewebsites.net/Match/getbyid/${matchId}`);
          if (!team1Response.ok) {
            throw new Error('Failed to fetch team details');
          }
          const MatchData = await team1Response.json();
          setMatch(MatchData);
          const teamResponse = await fetch(`https://link-up-tournament.azurewebsites.net/Team/getbyid/${MatchData.team1}`);
          if (!team1Response.ok) {
            throw new Error('Failed to fetch team details');
          }
          ...
          const team2Response = await fetch(`https://link-up-tournament.azurewebsites.net/Team/getbyid/${MatchData.team2}`);
          if (!team2Response.ok) {
            throw new Error('Failed to fetch team details');
          }
 ...
 /*   useEffect(() => {
      const fetchTeamDetails = async () => {
        try {
          console.log(match);
          const team1Response = await fetch(`https://link-up-tournament.azurewebsites.net/Team/getbyid/${match.team1}`);
          if (!team1Response.ok) {
            throw new Error('Failed to fetch team details');
          }
          ...
          const team2Response = await fetch(`https://link-up-tournament.azurewebsites.net/Team/getbyid/${match.team2}`);
          if (!team2Response.ok) {
            throw new Error('Failed to fetch team details');
          }
          const team2Data = await team2Response.json();
...
const onSubmit = async (formDataToSend) => {
  try {
    const response = await fetch(`https://link-up-tournament.azurewebsites.net/Match/update/${match._id}`, {
      method: 'PUT',
      headers: {
