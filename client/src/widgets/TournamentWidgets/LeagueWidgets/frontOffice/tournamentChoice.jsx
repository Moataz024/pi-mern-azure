...
const TournamentChoice = ({ selectedTournamentId }) => {
        ...
        const fetchTournamentDetails = async () => {
            ...
            try {
                if (selectedTournamentId){
                    const response = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/getbyid/${selectedTournamentId}`);
                    if (!response.ok) {
    ...
    useEffect(() => {
        const fetchMatchesForLeague = async () => {
            try {
                if (tournament._id) {
                    const response = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/fixturesByDay/${tournament._id}/${selectedDay}`);
                    if (!response.ok) {
    ...
    useEffect(() => {
        const fetchMatchesForKnockout = async () => {
            try {
                if (tournament._id) {
                    const response = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/FixturesByDayKnockout/${tournament._id}/${selectedDay}`);
                    if (!response.ok) {
        ...
        const fetchMatchResult = async (matchId) => {
            try {
                const response = await axios.get(`https://link-up-tournament.azurewebsites.net/result/resultMatch/${matchId}`);
    ...
    useEffect(() => {
        const fetchMatchesForChampionship = async () => {
            try {
                if (tournament._id) {
                    const response = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/MatcheGroupsByday/${tournament._id}/${selectedDay}`);
                    if (!response.ok) {
                        throw new Error('Failed to fetch matches for Knockout');
                    }
                    const data = await response.json();
