...
const RoundsDisplay = ({ selectedTournamentId }) => {
    ...
    useEffect(() => {
        const fetchFirstRoundMatches = async () => {
            try {
                if (tournament._id) {
                    const response = await fetch(`https://link-up-tournament.azurewebsites.net/Tournament/KnockoutTournament/${tournament._id}`);
                    if (!response.ok) {
    ...
    const fetchTeamDetails = async (teamId) => {
        try {
            const response = await fetch(`https://link-up-tournament.azurewebsites.net/Team/getbyid/${teamId}`);
            if (!response.ok) {
    ...
    const fetchMatchDetails = async (matchId) => {
        try {
            const response = await fetch(`https://link-up-tournament.azurewebsites.net/Match/getbyid/${matchId}`);
            if (!response.ok) {
