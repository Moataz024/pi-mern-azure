...
const PlayerListWithNoTeam = () => {
    ...
    const addNewPlayer = async ( newPlayerData) => {
        ...
        try {
            const response = await fetch(`https://link-up-tournament.azurewebsites.net/Team/addPlayer/${userId}`, {
                method: "POST",
                headers: {
    ...
    useEffect(() => {
        async function fetchPlayerData() {
            try {
                const response = await fetch(`https://link-up-tournament.azurewebsites.net/User/getallPlayersWithNoTeam`);
                const data = await response.json();
                setPlayerData(data);
            } catch (error) {
                console.error(error);
