...
const Team = () => {
  ...
  const checkTeam_manager = async (coachId) => {
    try {
      const response = await fetch(`https://link-up-tournament.azurewebsites.net/Team/checkTeam_manager/${coachId}`);
  ...
  async function updateTeam(coachId) {
    try {
      const response = await fetch(`https://link-up-tournament.azurewebsites.net/Team/updateXTeam/${coachId}`, {
        method: 'PUT',
      });
      return true; 
    } catch (error) {
  ...
  const addNewTeam = async (newTeamData) => {
    try {
      ...
      const response = await fetch(`https://link-up-tournament.azurewebsites.net/Team/add/${userId}`, {
        method: "POST",
        headers: {
