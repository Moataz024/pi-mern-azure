...
export const useWatchStream = () => {
    ...
    const getStreamUrl = async () =>{
        try{
            ...
            const response = await fetch('https://link-up-tournament.azurewebsites.net/stream/playing',{
                method:'GET',
                headers:{
    ...
    const joinStream = async () => {
        try{
            setIsLoading(true)
            setError('')
            const tokenResp = await fetch('https://link-up-tournament.azurewebsites.net/stream', {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                },
