...
const socket = io('https://link-up-tournament.azurewebsites.net', { transports : ['websocket'] });
const MatchResult = () => {
  ...
  const team2=teams?.team2?.team2
    useEffect(() => { 
        ...
        const fetchResult = async () => {
          try {
            const response = await axios.get(`https://link-up-tournament.azurewebsites.net/result/result/${MatchIdfromLeaguesDisplay}`); // Assuming you have an endpoint to get the current result
            console.log("aaaa",response.data);

            setResult(response.data);
            const teamsResponse = await axios.get(`https://link-up-tournament.azurewebsites.net/result/teams/${MatchIdfromLeaguesDisplay}`);
